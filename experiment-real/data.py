...
def get_dataset(experiment_name, save_dir="./experiment-real", test_split=0.8):
    '''Downloads and formats the datasets provided in the supplementary materials of
    the 2009 Lipson Science article "Distilling Free-Form Natural Laws from
    Experimental Data."
    Link to supplementary materials: https://bit.ly/2JNhyQ8
    ...
    os.makedirs(save_dir) if not os.path.exists(save_dir) else None
    out_file = f"{save_dir}/invar_datasets.zip"  # Using the local path

    # No need to download the file since it's already downloaded
    # urlretrieve(url, out_file)
